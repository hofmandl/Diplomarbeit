...
Handelstrategien bezeichen wir mit $\mathcal{A}(t,x)$. Crashprozesse sind alle auf $\Omega$ definierten Prozesse, die an die Filtrierung der Brownschen Bewegung adaptiert
...
\begin{myDefinition}
Sei $n \in \{0,1\}$. Wir bezeichnen
\begin{equation}
 J^n(t,x,\pi,N) := E_{t,x,n} \left[ U(X(T)) \right] := E \left[ U(X(T)) | X(t) = x, N(t)=1-n \right] 
\end{equation}
als Wertfunktion,
\begin{equation}
 V^n(t,x)= \sup_{\pi \in \mathcal{A}} \inf_{N \in \mathcal{B}} J^n(t,x,\pi,N)
\end{equation}
als optimale Wertfunktion und eine Handelsstrategie $\pi'$ für die
\begin{equation}
\sup_{\pi \in \mathcal{A}} \inf_{N \in \mathcal{B}} E \left[ U(X(T)) \right] 
=  \inf_{N \in \mathcal{B}} E \left[ U(X_{\pi'}(T)) \right] 
\end{equation}
gilt, als optimale Strategie.
\end{myDefinition}
% In der Notation von Fleming und Soner ist das also ein Statisches Spiel

Für $n=0$ befinden wir uns offenbar in der Situation von ??? und dafür kennen wir schon einen Verifikationssatz für die Wertfunktion und die optimale Strategie. Im folgenden wird
es unser Ziel sein, einen Analogen Verifikationssatz für $n=1$ zu formulieren und zu beweisen.
...
\begin{mySatz}
 \begin{align}
 V^1(t,x) &=  \sup_{\pi \in \mathcal{A}} \inf_{N \in \mathcal{B}}    E_{t,x,n} \left[ U(X^{\pi \tau}(T)) \right] \\
          &=  \inf_{N \in \mathcal{B}}   \sup_{\pi \in \mathcal{A}}  E_{t,x,n}  \left[ U(X^{\pi \tau}(T)) \right] \\
...
%          &=  \sup_{\pi \in \mathcal{A}} \inf_{\tau}  E_{t,x,n}  \left[  V^0(\tau,X^{\pi \tau}(\tau))  \right]\\
          &=  \inf_{\tau}   \sup_{\pi \in \mathcal{A}}  E_{t,x,n}  \left[  V^0(\tau,X^{\pi \tau}(\tau))  \right]\\
\end{align}
\end{mySatz}
...
\begin{proof}
Wir beschreiben zunächst die Struktur des Beweise: Mit Hilfe der Ito-Formel können wir eine Relation zwischen $v(t,x)$ und den Werten von $v$ bis zum Crash herleiten (vergleiche
\ref{itov}). Durch Angabe einer optimalen Strategie und aus den Vorrausetzungen des Satzes gelingt es dann eine obere Schranke für $v(t,x)$ (vergleiche
\ref{eq:erstewichtigeUngleichung} anzugeben, und genauso gelingt es durch Angabe eines optimalen Crashes eine untere Schranke für $v(t,x)$ (vergleiche
\ref{eq:erstewichtigeUngleichung}) herzuleiten. Diese Schranken falllen zusammen und charakterisieren nach dem vorangegangen Lemma die optimale Wertfunktion. Das die optimale
...
\begin{equation}
 \pi^1_t=\frac{1}{k} (1- \pi^1 k) 
  ...
  \right)
\end{equation}
mit $\pi^1(T)=0$ und das dazugehörige $f$ mit
\begin{equation} 
f^1_t(t) =  -(r+\pi^1(\mu-r)) + \einhalb (\pi^1)^2 \sigma^2
\end{equation}
und $f(T)=0$ die optimale Wertfunktion
\begin{equation}
 V^1(t,x) = \log(x) + f^1(t)
\end{equation}
alle Vorraustezngen des Verifikationsatzes erfüllen.

\section{Vergleich der beiden Ansätze}
Zunächst einige Bemerkungen zum stochastscihen Modell:
