...
Strategien dürfen wir uns dann auf solche beschränken, die nach unten durch $0$ beschränkt sind. Wir zeigen dieses Aussage nur für logaritmischen Nutzen. Der Autor hat das starke
Gefühl, dass die Aussage für beliebige Nutzenfunktionen richtig ist, kann es aber nicht streng beweisen. Wir werden auf die diesbezüglich kritischen Stellen im Beweis aufmerksam
machen.
\begin{mySatz}[Keine Notwendigkeit von Leerverkäufen, \cite{Korn2002}, 2.3. (b)] \label{th:UnnützeLeerverkäufe}
Für logaritmischen Nutzen gilt: $WC(\pi^{+},t,x) \geq WC(\pi,t,x)$
\end{mySatz}
\begin{proof}
...
Wir formen die Differentialgleichung  zunächst zu
\begin{equation*} 
...
\end{equation*}
Dabei haben wir die linke Seite mit Hilfe eines Computeralgebrasystem integriert, durch (langwieriges) Ableiten und Termzusammenfassen kann das Ergebnis überprüft werden.
...
\end{equation*}
können wir die erahltene Gleichung als
...
\end{equation*}
Für $t<T$ ist die rechte Seite von Gleichung \eqref{eq:nichtlineareGleichung} um $ \frac{\sigma^2}{2k}(T-t)$ größer als die linke Seite. Die Ableitung der linken Seite ist
\begin{equation*}
 \frac{1}{(1-\pi k) \left( \pi-\pistar \right)^2 },
\end{equation*}
(sie ist ja durch Integrieren dieses Termes entstanden), also ist sie für $\pi <\frac{1}{k}$ immer streng positiv. Die Gleichung \eqref{eq:nichtlineareGleichung} hat einen ersten
Pol bei $\min(\pi,1/k)$ mit dem Wert $\infty$, es gibt also genau einen Wert für $\pi$ der die Gleichung löst. Das beweist die Existenz und Eindeutigkeit und die Behauptung
$\pidach(0)<\min(\pi,1/k)$. Das $\pidach$ eine monoton fallende Funktion ist folgt daraus, dass die rechte Seite von  Gleichung \eqref{eq:nichtlineareGleichung} in $t$ fällt.
\end{proof}
Ein Bild von $\pidach$ für Beispielparameter findet man in Abbildung \ref{fig:optimaleStrategie}.
...
\begin{proof}
Da $\pidach$ die Differentialgleichung aus Satz \ref{th:DGL} erfüllt gilt:
\begin{equation*}
...
Die Lösung $\pidach$ der Differentialgleichung aus Satz \ref{th:DGL} ist eine Lösung des Worst"=Case"=Portfoliooptimierungsproblems.
\end{mySatz}
...
\end{equation*}
gilt. Wir gliedern diesen etwas längeren Beweis nun in 5 Unterpunkte.
\begin{enumerate}
 \item 
...
expliziten Form des erwarteten logarithmischen Nutzens (Satz \ref{th:ExpLogUtility}) und der Voraussetzung des höheren erwarteten Endnutzens von $\pi$ in diesem Fall. Dann folgt
...
führen folgende Rechnung für ein $t \geq t_0$ durch (Erklärungen folgen)
\begin{equation*}
\begin{split}
...
Nehmen wir nun davon den Erwartungswert, so fällt das Integral bezüglich der Brownschen Bewegung dank der Beschränktheit von $\pidach$ weg.
...
\end{equation*}
Dabei ist $\vdach(t_0,x_0)$ der erwartete logarithmische nutzen im crashfreien Scenario bei Verfolgen der Handelsstrategie $\pidach$. Es herrscht also eine Indifferenz gegenüber
den verschieden Scenarien. Ein Crash zu einem beliebigen Zeitpunkt als auch gar kein Crash führen stets zum gleichen erwarteten Endnutzen.
...
\begin{align*}
 WC(\pi,t_0,x_0) &\leq  E \left[ V^0(\tau,X_{\pi}(\tau) (1-\pi(\tau)k)) \right] \\
          ...
          &= WC(\pidach,t_0,x_0) 
\end{align*}
Die erste Abschätzung folgt aus der Tatsache, dass die \wcs kleiner als der erwartete Nutzen bei einem Crash zur Zeit $\tau$ ist. Die darauf folgende Gleichung ist
die explizite Form von $V^0$ aus Satz \ref{th:LogWertfunktion}. Die nächste Ungleichung folgt dann aus \eqref{dumdum} und
...
\end{equation*}
Dabei folgt die erste Ungleichung aus der Definition der Worst-Case-Schranke, die zweite aus der Annahme für diesen Fall und die letzte Gleichung ist die bekannte
Worst-Case-Schranke für $\pidach$. Damit sind alle Fälle behandelt und wir haben gezeigt dass jede beliebige Handelsstrategie $\pi$ keine bessere Worst-Case-Schranke als $\pidach$
besitzen kann.
\end{enumerate}
\end{proof}
...
Bondstrategie. Dass tun wir nun, indem wir eine Strategie angeben, die eine höhere Worst-Case-Schranke als die
